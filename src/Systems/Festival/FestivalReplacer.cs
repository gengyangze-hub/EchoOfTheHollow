...
namespace EchoesOfTheHollow.Systems.Festival
{
    ...
    public class FestivalReplacer
    {
        ...
        /// <summary>Check if today is a festival and trigger replacement</summary>
        public void CheckAndTriggerToday()
        {
            string key = $"{Game1.currentSeason}{Game1.dayOfMonth}";
            if (FestivalReplacements.TryGetValue(key, out string? name))
            {
                OnFestivalDay(name);
            }
        }
