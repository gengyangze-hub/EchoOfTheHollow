...
namespace EchoesOfTheHollow.Systems.Economy
{
    ...
    public class DriftBoxSystem
    {
        ...
        public void CheckNpcFindings()
        {
            ...
            string itemDisplayName = driftItem.ItemId;
            try
            {
                var resolved = ItemRegistry.Create(driftItem.ItemId);
                if (resolved != null) itemDisplayName = resolved.DisplayName;
            }
            ...
            {
                NpcName = finder,
                DisplayText = $"在{driftItem.DestinationLocation}发现了一个{itemDisplayName}。它看起来是被什么人放出去的，想看看谁会捡到它。好吧，现在我捡到了。也许它的主人有一天会在日志里读到这段话。",
        ...
        /// <summary>Remove drift records older than 30 days to prevent modData overflow</summary>
        private void CleanupExpiredRecords()
        {
            int today = (int)Game1.stats.DaysPlayed;
            int before = _driftItems.Count;
            _driftItems.RemoveAll(d => (d.IsFound && d.DriftDay < today - 14) || d.ExpiryDay < today - 30);
            if (before != _driftItems.Count)
                _monitor.Log($"[DriftBox] Cleaned {before - _driftItems.Count} old records.", LogLevel.Debug);
        }
