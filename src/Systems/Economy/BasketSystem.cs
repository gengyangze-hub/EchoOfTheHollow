...
namespace EchoesOfTheHollow.Systems.Economy
{
    ...
    public class BasketSystem
    {
        ...
        /// <summary>Remove returned/matched basket items older than 30 days</summary>
        private void CleanupOldRecords()
        {
            int today = (int)Game1.stats.DaysPlayed;
            int before = _items.Count;
            _items.RemoveAll(i => i.Status != ExchangeStatus.Pending && i.DepositedDay < today - 30);
            if (before != _items.Count)
                _monitor.Log($"[Basket] Cleaned {before - _items.Count} old records.", LogLevel.Debug);
        }
